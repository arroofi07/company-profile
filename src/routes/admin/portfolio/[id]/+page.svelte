<script lang="ts">
	...
	let tags = $state<string[]>(data.portfolio?.tags || []);
...
<div class="max-w-4xl mx-auto space-y-6">
	...
	<div class="flex items-center gap-4">
		...
		<div>
			<h1 class="text-2xl font-bold text-text-primary">Edit Portfolio</h1>
			<p class="text-text-secondary mt-1">Perbarui informasi project</p>
		</div>
	</div>
	...
	{#if data.portfolio}
		...
		>
			...
			<div>
				...
				<ImageUpload
					value={data.portfolio.image_url}
					onchange={(file) => {
						imageFile = file;
						imageChanged = true;
					}}
			...
			<div>
				...
				>{data.portfolio.description_id}</textarea>
			</div>
			...
			<div>
				...
				>{data.portfolio.description_en}</textarea>
			</div>
			...
			<div class="flex flex-wrap gap-6">
				<label class="flex items-center gap-3 cursor-pointer">
					<input
						type="checkbox"
						name="featured"
						checked={data.portfolio.featured}
				...
				<label class="flex items-center gap-3 cursor-pointer">
					<input
						type="checkbox"
						name="published"
						checked={data.portfolio.published}
						class="w-5 h-5 rounded border-border-subtle bg-bg-tertiary text-purple-500 focus:ring-purple-500 focus:ring-offset-0"
					/>
